...
class User(db.Model):
    ...
    # children
    # cardratings are not deleted on user delete
    # flashcards are not deleted on user delte
    flashcards = db.relationship("Flashcard",  backref="user")
    # peerreviews are deleted on user delete, but cardratings are kept.
...
def user_registered(email, username):
    ...
    if (len(user_list) > 1):
        raise Exception(
            "Error. Multiple users registered with email and username")
    if (len(user_list) == 0):
        return False
    return True
...
def get_user_id(email, username):
    if not user_registered(email, username):
        raise Exception("Could not find user registered with email",
