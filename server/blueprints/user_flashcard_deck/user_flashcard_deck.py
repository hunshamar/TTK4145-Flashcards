...
class FlashcardReview(db.Model):
    ...
    flashcard_deck = db.Column(db.Integer, db.ForeignKey('user_flashcard_deck.id'))

    # flashcard 
    flashcard_id = db.Column(db.Integer, db.ForeignKey('flashcard.id'))
    flashcard = db.relationship("Flashcard", lazy='subquery')

    def move_to_back_of_deck(self):
        deck = UserFlashcardDeck.query.get(self.flashcard_deck)
        last_order = FlashcardReview.query.filter_by(flashcard_deck=self.flashcard_deck).order_by(FlashcardReview.order.desc()).first().order
...
class UserFlashcardDeck(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(256))
    flashcard_reviews = db.relationship("FlashcardReview", backref="user_flashcard_deck", cascade="all,delete", lazy='subquery')
    ...
    def __init__(self,title, user_id, flashcards):
        ...
        self.flashcard_reviews = [create_flashcard_review(flashcard,count) for count, flashcard in enumerate(flashcards)]
...
def answer_review(correct, flashcard_deck_id, card_id, user_id):
    ...
    flashcard_reviews = FlashcardReview.query.filter_by(flashcard_deck=flashcard_deck_id, flashcard_id=card_id).all()
...
def create_user_flashcard_deck(title, user_id, flashcards, number_of_cards):
    ...
    if len(title) > max_chacacters:
        raise Exception(f"Title too long ({len(title)} characters). Max {max_chacacters} characters")
    ...
    got_flashcards = [Flashcard.query.get(f["id"]) for f in flashcards if Flashcard.query.get(f["id"]).collective_deck_id]
