...
import { getRating, saveRating, saveDifficultyRating, saveQualityRating, saveDuplicatesRating } from '../../store/actions/ratingActions';
...
const RateCard = ({ rating, save, previewCard, cardIdToIndex }) => {
    ...
    const rating_state = useSelector(state => state.ratingReducer.ratings.find(rat => rat.id == rating.id))
    const otherRatings = useSelector(state => state.ratingReducer.ratings.filter(rat => rat.id !== rating.id))
            ...
            <Grid className={classes.container} container spacing={2} >
                ...
                <Grid item xs={3}>
                    ...
                    <Box style={{ minHeight: "30px", minWidth: "30px" }}>
                        <Typography variant="body2" color="textSecondary">{duplicateRatings.map(rating => <b>#{rating.index}, </b>)} </Typography>
                    </Box>
